  ...
  const openAttendees = async (event: OrgEvent) => {
    ...
    try {
      ...
      const res = await fetch(
        `${BACKEND_URL}/api/events/${event.id}/attendees`,
        {
          headers: token ? { Authorization: `Bearer ${token}` } : {},
        }
      );
      ...
      <div className="pb-20 max-w-4xl mx-auto px-4 py-10">
        ...
        <div className="flex gap-4 items-start mb-8 p-5 bg-white rounded-2xl border border-gray-100 shadow-sm">
          {selectedEvent.banner_image_url ? (
            ...
            <div className="h-20 w-32 rounded-xl bg-amber-50 flex items-center justify-center text-amber-300 text-4xl font-black shrink-0">
          ...
          <div className="min-w-0">
            <span
              className={`text-[11px] font-bold uppercase px-2 py-0.5 rounded-full ${
                categoryColors[selectedEvent.category?.toLowerCase()] ??
            ...
            <div className="flex flex-wrap gap-4 mt-2 text-xs text-gray-400">
              ...
              <span className="flex items-center gap-1">
                <Users size={12} />
                {selectedEvent.attendee_count ?? 0} joined
              </span>
            </div>
          </div>
        </div>
          ...
          {attendees.length > 0 && (
            <div className="relative">
              ...
              <input
                ...
                className="pl-8 pr-4 py-2 rounded-xl border border-gray-200 text-sm outline-none focus:ring-2 focus:ring-amber-500 w-56"
    ...
    <div className="pb-20 max-w-5xl mx-auto px-4 py-10">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Events you've created. Click an event to view attendees.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="py-20 text-center rounded-3xl border border-dashed border-gray-200 bg-white">
          ...
          <a
            href="#/create-event"
            className="mt-6 inline-block px-6 py-3 bg-amber-600 text-white text-sm font-bold rounded-2xl hover:bg-amber-700 transition-colors"
          >
            Create Event
          </a>
        </div>
        ...
        <div className="space-y-3">
          {events.map((event) => {
            ...
            return (
              <div
                key={event.id}
                className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:border-amber-100 transition-all"
              >
                <div className="flex gap-4 items-center p-4">
                  ...
                  {event.banner_image_url ? (
                    ...
                    <div className="h-16 w-24 rounded-xl bg-amber-50 flex items-center justify-center text-amber-300 text-3xl font-black shrink-0 select-none">
                  ...
                  <div className="flex flex-col items-end gap-2 shrink-0">
                    ...
                    <button
                      ...
                      title="Edit event"
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-gray-100 text-gray-600 hover:bg-amber-50 hover:text-amber-700 text-xs font-semibold transition-colors"
                    >
                      <Pencil size={12} /> Edit
                    </button>
                  </div>
                ...
                {isToday && event.status === "published" && (
                  <div className="px-4 pb-4">
                    <button
                      onClick={() => {
                        window.location.hash = `/checkin/${event.id}`;
                      }}
                      className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-amber-600 text-white text-sm font-bold hover:bg-amber-700 transition-colors"
                    >
